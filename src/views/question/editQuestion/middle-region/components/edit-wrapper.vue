<template>
    <div class="wrapper" :class="{ active: isActive }" @mousedown="moveSeriesEvent">
        <slot></slot>
        <!-- 拖拽大小的四个点 -->
...
const $emit = defineEmits<{
    ...
    (e: 'updateWidgetProps', params: { Key: keyof AllWidgetProps, Value: string | number | boolean }): void
}>()
...
const onMove = function (originalE: MouseEvent, documentEvent: MouseEvent) {
    ...
    const goalWrapper = (originalE.target as HTMLElement).closest('.wrapper') as HTMLElement
    if (goalWrapper) {
...
const onMoveEnd = function (originalE: MouseEvent) {
    ...
    const goalWrapper = (originalE.target as HTMLElement).closest('.wrapper') as HTMLElement
    // 发送仓库，持久化修改
    $emit('updateWidgetProps', { Key: 'left', Value: goalWrapper.style.left })
    $emit('updateWidgetProps', { Key: 'top', Value: goalWrapper.style.top })
...
const onResizeStart = function (originalE: MouseEvent) {
    ...
    console.log('resize', originalE.target)
    handleClick()
    moveData.start.X = originalE.clientX
    moveData.start.Y = originalE.clientY
}
const onResizeProcess = function (originalE: MouseEvent, documentEvent: MouseEvent) {
    ...
    const goalWrapper = goalPoint.closest('.wrapper') as HTMLElement
    const goalWidget = goalPoint.closest('.wrapper')?.firstElementChild as HTMLElement
    /*
...
const onResizeEnd = function (originalE: MouseEvent) {
    ...
    switch (true) {
        case goalPoint.classList.contains('point1'):
            $emit('updateWidgetProps', { Key: 'top', Value: goalWrapper.offsetTop + 'px' })
            $emit('updateWidgetProps', { Key: 'left', Value: goalWrapper.offsetLeft + 'px' })
            break
