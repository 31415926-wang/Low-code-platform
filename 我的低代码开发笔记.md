...
15.公共组件库的搭建
  ...
  19.上线、线上问题修复
    ...
    -线上获取DOM问题（属性挂载不上，丢失id的问题）：不明原因，手动挂载

        子组件接收后的属性key情况：
        有prop接收：
        1.如果key传入时为驼峰命名，会保留原名
        2.如果key传入时为-命名，会自动驼峰

        没有prop接收：（属性会直接挂载到根元素上）
        1.如果key传入时为驼峰命名，会自动全部小写
        2.如果key传入时为-命名，会保留原名

  
    -本地打包没问题，但是CI/CD工作流中打包出现找不到模块：
       确保在 CI/CD 环境中，文件路径的大小写与本地一致。有些操作系统（如 Windows）对文件名不区分大小写，而其他（如 Linux）则区分。因此，即使在本地运行正常，CI/CD 中可能会因为路径不匹配而出错。


    -使用render函数+appendChild添加组件，组件内不能获取store问题 （棘手问题）
      1.与执行顺序无关
      2.与是否在app容器内无关
